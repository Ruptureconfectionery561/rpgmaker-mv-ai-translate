...
class OpenAITranslator(BaseTranslator):
    """
    Communicates with an OpenAI-compatible API (like LM Studio) to translate blocks of text.
    Expects input and outputs to be formatted in numbered blocks.
    """
    
    def __init__(self, config: TranslationConfig):
        super().__init__(config)
        self.api_url = f"{self.config.endpoint.rstrip('/')}/v1/chat/completions"
    ...
    def _parse_response(self, response_text: str, expected_count: int) -> List[str]:
        ...
        if len(translations) != expected_count:
            ...
            logger.error(f"API returned {len(translations)} blocks, expected {expected_count}.")
    ...
    def translate_batch(self, texts: List[str]) -> List[str]:
        ...
        try:
            ...
            logger.error(f"OpenAI API request timed out (is the LLM server running at {self.api_url}?): {e}")
            raise TranslationError(f"API Request timed out: {e}") from e
        except requests.RequestException as e:
            logger.error(f"OpenAI API request failed (check endpoint URL): {e}")
